            ...
            if not event.is_directory:
                ...
                if (
                    ...
                    try:
                        ...
                        tracked_set.discard(str(path))
                    except Exception as exc:
                        logging.error(
                            "Failed to clean up deleted file %s: %s", path, exc
                        )

        def on_moved(self, event):
            if not event.is_directory:
                src_path = Path(event.src_path)
                if str(src_path) in tracked_set:
                    try:
                        rel = src_path.relative_to(source_dir)
                        dst = workspace_dir / rel
                        if dst.exists():
                            dst.unlink()
                            logging.info("Removed moved file %s", rel)
                        tracked_set.discard(str(src_path))
                    except Exception as exc:
                        logging.error(
                            "Failed to clean up moved file %s: %s", src_path, exc
                        )
            with lock:
...
def _run_polling(
    ...
    hr_port_file = source_dir / ".hot_reload_port"
    hr_port_file_existed = hr_port_file.exists()

    while True:
        time.sleep(_POLL_INTERVAL)

        # Check if .hot_reload_port just appeared
        if not hr_port_file_existed and hr_port_file.exists():
            hr_port_file_existed = True
            _render(source_dir, workspace_dir)
